...
hooksecurefunc(NamePlateDriverFrame, "OnNamePlateCreated", function(self, base)--  Hook Nameplate creation
    if ( unit ~= "player" and AbyssUIClassicAddonSettings.ExtraFunctionNameplateChanges ~= true ) then
...
hooksecurefunc("CompactUnitFrame_UpdateHealth", function(self)-- This is a shared function with other UnitFrames
    if ( NamePlateHealthText[self] and AbyssUIClassicAddonSettings.ExtraFunctionNameplateChanges ~= true ) then 
...
SquareMinimap_:SetScript("OnEvent", function(self, event, ...)
	if ( event == "PLAYER_ENTERING_WORLD" ) then
		...
		if ( AbyssUIClassicAddonSettings.DisableSquareMinimap ~= true ) then
			...
			local mediaFolder = "Interface\\AddOns\\AbyssUIClassic\\Textures\\minimap\\"	-- don't touch this ...
			...
			if moveWatchFrame then
				...
				QuestWatchFrame:SetPoint(qanchor, qparent, qanchor, qposition_x, qposition_y)
				QuestWatchFrame.SetPoint = function() end
				QuestWatchFrame:SetClampedToScreen(true)
				QuestWatchFrame:SetHeight(qheight)
			end
